...
function openNav() {
    ...
    if ($(window).width() < 451 || $(window).height() < 451) $(".aboutDesc").css("text-align", "center");
}
...
function closeNav() {
    ...
    if ($(window).width() < 451 || $(window).height() < 451) $(".aboutDesc").css("text-align", "justify");
}

$(".Hover").hover(function () {
    $("." + $(this).attr('class').split(" ")[1].toString()).css("background-color", "#cbcbcb");
    $("." + $(this).attr('class').split(" ")[1].toString()).css("font-weight", "500");
}, function () {
    $("." + $(this).attr('class').split(" ")[1].toString()).css("background-color", "unset");
    $("." + $(this).attr('class').split(" ")[1].toString()).css("font-weight", "unset");
});
...
function scrolling() {
    ...
    if (!navOpen) {
        ...
        if (topScreen > topTitle && topScreen - topTitle < 200 && !scrol && $(window).width() > 450 && $(window).height() > 521) {
            openNav();
            scrol = true;
            $('body').addClass('stop-scrolling');
            setTimeout(() => {
        ...
        } else if (topAbt <= botScreen && !scrol && $(window).width() > 450 && $(window).height() > 521) {
            openNav();
        }
...
function headerClick(index) {
    ...
    $(".sHeader").removeClass("Head" + Type).addClass("Head" + goTo);
